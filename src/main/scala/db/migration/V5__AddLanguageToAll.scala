...
class V5__AddLanguageToAll extends JdbcMigration {
  ...
  def allLearningPaths(implicit session: DBSession): Seq[V5_LearningPath] = {
    sql"select id, document from learningpaths".map(rs => {
      ...
      meta.copy(
        ...
        description = meta.description.map(d => V5_Description(d.description, Some(Language.languageOrUnknown(d.language)))),
  ...
  def allLearningSteps(implicit session: DBSession): List[V5_LearningStep] = {
    sql"select id, document from learningsteps".map(rs => {
      val meta = read[V5_LearningStep](rs.string("document"))
      meta.copy(
        id = Some(rs.long("id")),
        title = meta.title.map(t => V5_Title(t.title, Some(Language.languageOrUnknown(t.language)))),
        description = meta.description.map(t => V5_Description(t.description, Some(Language.languageOrUnknown(t.language)))),
        embedUrl = meta.embedUrl.map(t => V5_EmbedUrl(t.url, Some(Language.languageOrUnknown(t.language)), t.embedType))
      )
    }).list().apply()
  }
