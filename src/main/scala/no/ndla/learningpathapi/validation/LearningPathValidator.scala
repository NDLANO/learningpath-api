...
trait LearningPathValidator {
  ...
  class LearningPathValidator(titleRequired: Boolean = true,
    ...
    def validate(newLearningPath: LearningPath,
                 allowUnknownLanguage: Boolean = false): LearningPath = {
    ...
    def validate(updateLearningPath: UpdatedLearningPathV2): Unit = {
      languageValidator.validate("language",
                                 updateLearningPath.language,
                                 allowUnknownLanguage = false) match {
        case None =>
    ...
    private def validateDuration(
        durationOpt: Option[Int]): Option[ValidationMessage] = {
      durationOpt match {
        case None           => None
    ...
    private def validateCopyright(
        copyright: Copyright): Seq[ValidationMessage] = {
    ...
    private def validateLicense(license: String): Seq[ValidationMessage] = {
      getLicense(license) match {
        case None =>
          Seq(
            new ValidationMessage("license.license",
                                  s"$license is not a valid license"))
        case _ => Seq()
      }
    }
