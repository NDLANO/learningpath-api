...
object JestClientFactory {
  def getClient(searchServer: String = LearningpathApiProperties.SearchServer): NdlaJestClient = {
    LearningpathApiProperties.RunWithSignedSearchRequests match {
  ...
  private def getSigningClient(searchServer: String): NdlaJestClient = {
    ...
    val awsSigner = new AWSSigner(new DefaultAWSCredentialsProviderChain(), LearningpathApiProperties.SearchRegion, "es", clock);
