...
trait SearchServiceComponent extends LazyLogging {
  this: SearchIndexServiceComponent
    with Elastic4sClient
    with SearchConverterServiceComponent =>
  val searchService: SearchService

  class SearchService {
    def getHitsV2(response: SearchResponse,
                  language: String): Seq[LearningPathSummaryV2] = {
    ...
    def hitAsLearningPathSummaryV2(hitString: String,
                                   language: String): LearningPathSummaryV2 = {
      implicit val formats = org.json4s.DefaultFormats
      searchConverterService.asApiLearningPathSummaryV2(
        read[SearchableLearningPath](hitString),
        language)
    }
          ...
          boolQuery()
            .should(
              ...
              nestedQuery("learningsteps.descriptions", stepDescSearch)
                ...
                .boost(1)
                .inner(
                  innerHits("learningsteps.descriptions").highlighting(hi)),
    ...
    def getStartAtAndNumResults(page: Option[Int],
                                pageSize: Option[Int]): (Int, Int) = {
          ...
          e.rf.status match {
            ...
            case _ => {
              ...
              throw new ElasticsearchException(
                s"Unable to execute search in ${LearningpathApiProperties.SearchIndex}",
                e.getMessage)
            }
          }
